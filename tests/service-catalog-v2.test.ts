...
import { DIGITAL_PROJECT_TYPES, FAI_SERVICE_CATALOG_V2, buildCatalogV2Snapshot, catalogV2RevisionHash, catalogV2Storage, validateCatalogSelection } from '../src/lib/service-catalog-v2';
import { catalogRevisionIsSelectable, prepareServiceCatalogV2, storedRevisionMatches } from '../src/lib/service-catalog-v2-persistence';
import { provisionCatalogBrowserFixtures } from './catalog-browser/provision-service';
...
test('persisted JSON comparison ignores object-key order but preserves values and array order', () => {
  const service = FAI_SERVICE_CATALOG_V2.find(({ code }) => code === 'ottimizzazione_aziendale_ai')!;
  const storage = catalogV2Storage(service);
  const detail = service.detail;
  const reorderedDetail = {
    subsequentEngagements: detail.subsequentEngagements,
    deliverables: detail.deliverables,
    timing: detail.timing,
    phaseDocuments: detail.phaseDocuments,
    exclusions: detail.exclusions,
    inclusions: detail.inclusions,
  };
  const operationalConditions = storage.operationalConditions.map((item) => typeof item === 'object'
    ? { detail: reorderedDetail, catalogVersion: service.sourceCatalogVersion }
    : item);
  const row = {
    version: service.revisionVersion, publicName: service.name, shortDescription: service.description,
    priceMode: service.priceMode, netPrice: null, currency: 'EUR', vatRateBps: 2200,
    validFrom: new Date(service.validFrom), termsVersion: service.termsVersion, checkoutEnabled: false,
    autoClientDeliveryAllowed: false, autoExternalActionAllowed: false, operationalConditions,
    checklist: storage.checklist, contentHash: catalogV2RevisionHash(service), status: 'PUBLISHED',
  };
  assert.equal(storedRevisionMatches(service, row as never), true);
  assert.equal(storedRevisionMatches(service, { ...row, operationalConditions: [...operationalConditions].reverse() } as never), false);
  assert.equal(storedRevisionMatches(service, { ...row, operationalConditions: operationalConditions.map((item) => typeof item === 'object' ? { ...item, catalogVersion: 'altered' } : item) } as never), false);
});

test('selection requires active service and the published canonical revision inside its validity interval', () => {
  const service = FAI_SERVICE_CATALOG_V2.find(({ code }) => code === 'ottimizzazione_aziendale_ai')!;
  const storage = catalogV2Storage(service);
  const row = { version: service.revisionVersion, publicName: service.name, shortDescription: service.description, priceMode: service.priceMode, netPrice: null, currency: 'EUR', vatRateBps: 2200, validFrom: new Date(service.validFrom), validUntil: null, termsVersion: service.termsVersion, checkoutEnabled: false, autoClientDeliveryAllowed: false, autoExternalActionAllowed: false, operationalConditions: storage.operationalConditions, checklist: storage.checklist, contentHash: catalogV2RevisionHash(service), status: 'PUBLISHED', serviceCatalog: { active: true } };
  const atStart = new Date(service.validFrom);
  assert.equal(catalogRevisionIsSelectable(service, row as never, atStart), true);
  assert.equal(catalogRevisionIsSelectable(service, { ...row, validFrom: new Date(atStart.getTime() + 1) } as never, atStart), false);
  assert.equal(catalogRevisionIsSelectable(service, { ...row, validUntil: atStart } as never, atStart), false);
  assert.equal(catalogRevisionIsSelectable(service, { ...row, serviceCatalog: { active: false } } as never, atStart), false);
});

test('browser provisioning performs zero writes unless configuration and physical sentinel are synthetic', { concurrency: false }, async () => {
  const keys = ['RUN_DB_TESTS', 'AI_ORCHESTRATOR_DB_TESTS_CONFIRMED', 'AI_ORCHESTRATOR_DB_TEST_SENTINEL', 'DATABASE_URL', 'APP_ENV', 'NODE_ENV', 'CATALOG_BROWSER_SYNTHETIC_CONFIRMED'] as const;
  const previous = Object.fromEntries(keys.map((key) => [key, process.env[key]]));
  let writes = 0;
  const db = (identity: { sentinel: string | null }) => ({
    $queryRaw: async () => [{ databaseName: 'fai_crm_test', schemaName: 'public', serverAddress: '127.0.0.1', ...identity }],
    user: { createMany: async () => { writes += 1; } },
    userPermissionOverride: { create: async () => { writes += 1; } },
  });
  const password = 'synthetic-catalog-password-only';
  try {
    process.env.RUN_DB_TESTS = '1'; process.env.AI_ORCHESTRATOR_DB_TESTS_CONFIRMED = '1'; process.env.AI_ORCHESTRATOR_DB_TEST_SENTINEL = 'FAI_CRM_EPHEMERAL_TEST_ONLY_V1'; process.env.CATALOG_BROWSER_SYNTHETIC_CONFIRMED = '1'; process.env.APP_ENV = 'test'; Reflect.set(process.env, 'NODE_ENV', 'test');
    process.env.DATABASE_URL = 'postgresql://postgres:test@example.invalid/fai_crm_test?schema=public';
    await assert.rejects(provisionCatalogBrowserFixtures(db({ sentinel: 'FAI_CRM_EPHEMERAL_TEST_ONLY_V1' }) as never, password), /SYNTHETIC_CONFIGURATION_DENIED/u);
    process.env.DATABASE_URL = 'postgresql://postgres:test@127.0.0.1:5432/fai_crm_test?schema=public'; Reflect.set(process.env, 'NODE_ENV', 'production');
    await assert.rejects(provisionCatalogBrowserFixtures(db({ sentinel: 'FAI_CRM_EPHEMERAL_TEST_ONLY_V1' }) as never, password), /SYNTHETIC_CONFIGURATION_DENIED/u);
    Reflect.set(process.env, 'NODE_ENV', 'test');
    await assert.rejects(provisionCatalogBrowserFixtures(db({ sentinel: null }) as never, password), /SYNTHETIC_DATABASE_DENIED/u);
    assert.equal(writes, 0);
    await provisionCatalogBrowserFixtures(db({ sentinel: 'FAI_CRM_EPHEMERAL_TEST_ONLY_V1' }) as never, password);
