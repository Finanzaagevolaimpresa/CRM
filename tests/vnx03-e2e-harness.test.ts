...
test('VNX-03 N14 phase keeps the legacy phase disabled and uses authentic registry sessions', () => {
  ...
  assert.match(browser, /first-response-headers[\s\S]*first-response-semantic-result/u);
  assert.match(browser, /first-response-request-started[\s\S]*deadlineSeconds: 30/u);
  assert.match(browser, /uiAppliedBeforeReload: true/u);
