...
test('VNX-03 scope guard admits N15 and harness-only but rejects protected or migration drift', () => {
  {
    const value = fixture();
    try {
  ...
  for (const scenario of [
    ...
    try {
      ...
      const existing = (() => { try { return readFileSync(join(value.repository, scenario.path), 'utf8'); } catch { return ''; } })();
      commit(value.repository, scenario.path, existing + scenario.contents);
