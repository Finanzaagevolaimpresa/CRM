...
async function waitForPreAnalysisForm(page: import('@playwright/test').Page) {
  await page.locator('form[data-preanalysis-form-ready="true"]').waitFor({ state: 'attached', timeout: 15_000 });
}
...
test('real manual pre-analysis path, conflict retention and access denials', async ({ browser }) => {
  ...
  await expect(page.locator('textarea[name="internalSummary"]')).toHaveValue('Richiesta alterata');
  await page.getByRole('button', { name: 'Crea bozza interna' }).click();
  await waitForPreAnalysisForm(page);
  ...
  await waitForPreAnalysisForm(secondTab);
  await page.goto(`${app}/preanalyses/${recordId}`);
  await waitForPreAnalysisForm(page);
  await page.locator('textarea[name="scenarioB"]').fill('Vincitore concorrente');
  await expect(page.locator('textarea[name="scenarioB"]')).toHaveValue('Vincitore concorrente');
  await page.getByRole('button', { name: 'Salva modifiche' }).click();
  await waitForPreAnalysisForm(page);
