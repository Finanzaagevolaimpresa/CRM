...
test('RBAC esterno è ristretto ad admin e direzione', () => {
  assert.equal(hasPermission({ role: 'admin' } as never, 'ai.external.run'), true);
  assert.equal(hasPermission({ role: 'direzione' } as never, 'ai.external.run'), true);
  for (const role of ['consulente', 'revisore', 'backoffice', 'commerciale', 'amministrazione', 'collaboratore_limitato'] as const) {
    assert.equal(hasPermission({ role } as never, 'ai.external.run'), false, `${role} non deve eseguire provider esterni`);
  }
});
