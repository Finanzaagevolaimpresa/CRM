...
La successiva Persistent Job Queue Foundation v1, finché resta in Draft e non distribuita, aggiunge soltanto lo schema di una coda passiva e un outbox transazionale. I job possono essere esclusivamente `PLANNED` o `BLOCKED`; l'outbox è esclusivamente `PENDING`. Non esistono `RUNNING`, claim, lease, worker, cron, unità systemd, consumer, dispatch, esecuzioni agente o provider call. La migration non cambia i flag, non crea workflow/job e non usa dati CRM reali. Contratto e controlli sono descritti in [Persistent Job Queue Foundation v1](ai-orchestrator-persistent-job-queue-foundation.md) e nell'[ADR-0002](adr/0002-ai-orchestrator-persistent-job-queue-foundation.md).
