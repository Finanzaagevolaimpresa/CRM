...
La Worker Runtime Foundation v1 proposta aggiunge soltanto runtime, attempt, receipt outbox, audit e primitive interne. Non installa un processo worker, script, timer o unità systemd; non crea `AiRun`, non applica transizioni e non invoca provider. Introduce il gate ambiente `AI_ORCHESTRATOR_WORKER_ENABLED`, che deve restare `0` in produzione. Anche `stateMachineEnabled` e `dispatchEnabled` devono restare `false`; la migration non ne modifica i valori. Dettagli e rollback sono descritti in [Worker Runtime Foundation v1](ai-orchestrator-worker-runtime-foundation.md) e nell'[ADR-0003](adr/0003-ai-orchestrator-worker-runtime-foundation.md).
