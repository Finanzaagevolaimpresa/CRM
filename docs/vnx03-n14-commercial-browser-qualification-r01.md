...
osservato entrambi gli invii, attende con deadline le due response e richiede un HTTP 200 e un HTTP
500 con media type RSC. Non usa l'EOF potenzialmente indefinito dello stream come confine: qualifica
il completamento applicativo con stato DB `claimed`, UI owner aggiornata, recupero navigabile del
browser che ha osservato l'errore e health HTTP del proxy. Il 500 prova soltanto l'esito HTTP. Una sonda
...
response HTTP 200 RSC dell'esatta server action, il checkpoint DB `contacted` e la scomparsa del
comando dalla UI prima del reload e della seconda verifica persistente.
...
I checkpoint diagnostici conservano soltanto fase, status, media type e indicatori booleani di UI,
health e stato persistente. Non salvano body RSC, header ulteriori, trace o video e non descrivono
come “completato” uno stream per il quale non è stato osservato l'EOF.
