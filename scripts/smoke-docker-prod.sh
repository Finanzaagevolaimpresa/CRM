...
# PR83 is test-only: every production entry/module must be free of its execution graph.
for production_module in \
  scripts/ai-orchestrator-worker.ts \
  src/lib/ai-orchestrator/worker-admission-claim-lease-process-v1.ts \
  src/lib/ai-orchestrator/worker-runtime-adapter-v1.ts; do
  if grep -Eq 'runtime-testing-composition|mock-execution-result-wiring|mock-handler-registry|consumeMockResult' "$production_module"; then
    echo "PR83 production execution graph unexpectedly wired in $production_module" >&2
    exit 1
  fi
done
...
[[ "$(compose exec -T postgres psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -Atc 'SELECT COUNT(*) FROM "_prisma_migrations" WHERE finished_at IS NOT NULL AND rolled_back_at IS NULL')" == "44" ]] \
  || fail "Production image did not apply exactly 44 migrations"
...
[[ "$(compose exec -T postgres psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -Atc "SELECT FORMAT_TYPE(attribute_row.atttypid, attribute_row.atttypmod) || '|' || attribute_row.attnotnull FROM pg_attribute attribute_row JOIN pg_class table_row ON table_row.oid = attribute_row.attrelid JOIN pg_namespace namespace_row ON namespace_row.oid = table_row.relnamespace WHERE namespace_row.nspname = 'public' AND table_row.relname = 'PrivacyEvidenceReceipt' AND attribute_row.attname = 'sourceSubmittedAt' AND attribute_row.attnum > 0 AND NOT attribute_row.attisdropped")" == "timestamp(3) with time zone|true" ]] \
  || fail "N13-C2 privacy evidence source timestamp type is not fail-closed timestamptz(3)"
[[ "$(compose exec -T postgres psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -Atc "SELECT REGEXP_COUNT(PG_GET_FUNCTIONDEF(function_row.oid), 'AT TIME ZONE ''UTC''') || '|' || REGEXP_COUNT(PG_GET_FUNCTIONDEF(function_row.oid), 'TO_CHAR\\(') FROM pg_proc function_row JOIN pg_namespace namespace_row ON namespace_row.oid = function_row.pronamespace WHERE namespace_row.nspname = 'public' AND function_row.proname = 'privacy_evidence_receipt_validate_v1'")" == "1|3" ]] \
...
[[ "$(compose exec -T postgres psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -Atc 'SELECT (SELECT COUNT(*) FROM "LeadIdentityKeyVersion") || '\''|'\'' || (SELECT COUNT(*) FROM "LeadIdentityKey") || '\''|'\'' || (SELECT COUNT(*) FROM "LeadProjectionLedger") || '\''|'\'' || (SELECT COUNT(*) FROM "LeadDuplicateCase") || '\''|'\'' || (SELECT COUNT(*) FROM "LeadDuplicateCandidate") || '\''|'\'' || (SELECT COUNT(*) FROM "LeadDuplicateDecision")')" == "0|0|0|0|0|0" ]] \
  || fail "N13 dormant deployment unexpectedly created projection or duplicate-resolution state"
...
echo "Docker production smoke test completed: 44 migrations, dormant VNX-01/N12/N13/N14/N15 paths, N04 privacy registries, production seed, app health, N03 report-only security headers, closed image optimizer, ai:reconcile, fail-closed worker gates, and cleanup succeeded for $COMPOSE_PROJECT_NAME."
