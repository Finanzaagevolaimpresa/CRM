...
`forward` verifies the same healthy source and project invariants at the mutation boundary, records an atomic hash chain before and after removal/create/start, and creates the selected app with `docker compose up --no-start --no-deps --no-build --pull never app` before starting its recorded ID. CI checks these flags against the installed Compose help. An absent candidate is returnable only after the closed observed interruption sequence; an inspect error, timeout or uncertain inventory is not authority.
