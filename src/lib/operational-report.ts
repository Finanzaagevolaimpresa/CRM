...
export async function buildOperationalReportMarkdown(session: AuthSession, input: { clientId?: string; technicalPracticeId?: string }) {
  ...
  const scopeWhere = practice ? { OR: [{ clientId }, ...serviceFilter, ...projectFilter] } : { clientId };
  const [services, projects, documents, checklist, tasks, communications, clientDossiers, aiOutputs, audits, catalog] = await Promise.all([
    ...
    hasPermission(session, 'dossier.read') ? prisma.clientDossier.findMany({ where: { clientId, ...(practice?.clientServiceId ? { clientServiceId: practice.clientServiceId } : {}) }, orderBy: { updatedAt: 'desc' }, take: 10 }) : Promise.resolve([]),
    hasPermission(session, 'ai.review') || hasPermission(session, 'ai.approve') ? prisma.aiOutput.findMany({ where: { OR: [{ clientId }, ...serviceFilter, ...projectFilter] }, orderBy: { createdAt: 'desc' }, take: 10 }) : Promise.resolve([]),
  ...
  return { title, markdown: [
    ...
    '', '## Dossier e output AI autorizzati', list([...clientDossiers.map((d) => ({ text: `Dossier: ${d.title} · ${clean(d.type)} · ${clean(d.status)} · aggiornato ${fmt(d.updatedAt)}` })), ...aiOutputs.map((o) => ({ text: `Output AI: ${o.title} · ${clean(o.status)} · creato ${fmt(o.createdAt)}` }))], (x) => `- ${x.text}`),
