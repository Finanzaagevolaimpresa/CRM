...
export async function createCommercialOfferAndRedirect(form: FormData) { await createCommercialOffer(form); const leadId=String(form.get('leadId')||''); revalidatePath(`/leads/${leadId}`); revalidatePath('/leads'); revalidatePath('/dashboard'); }
