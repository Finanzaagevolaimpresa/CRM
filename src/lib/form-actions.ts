...
export async function convertLeadToClientAndRedirect(form: FormData) { const client = await convertLeadToClient(form); const id=String(form.get('id')||''); revalidatePath(`/leads/${id}`); revalidatePath('/leads'); revalidatePath('/leads/inbox'); revalidatePath('/clients'); revalidatePath('/dashboard'); redirect(`/clients/${client.id}`); }
async function revalidateCommercialLeadInbox(form: FormData) { const id=String(form.get('id')||''); revalidatePath(`/leads/${id}`); revalidatePath('/leads'); revalidatePath('/leads/inbox'); }
export async function initializeCommercialLeadInbox(form: FormData) { await initializeCommercialLeadInboxAction(form); await revalidateCommercialLeadInbox(form); }
export async function claimCommercialLeadInbox(form: FormData) { await claimCommercialLeadInboxAction(form); await revalidateCommercialLeadInbox(form); }
export async function assignCommercialLeadInbox(form: FormData) { await assignCommercialLeadInboxAction(form); await revalidateCommercialLeadInbox(form); }
export async function unassignCommercialLeadInbox(form: FormData) { await unassignCommercialLeadInboxAction(form); await revalidateCommercialLeadInbox(form); }
export async function recordCommercialLeadFirstResponse(form: FormData) { await recordCommercialLeadFirstResponseAction(form); await revalidateCommercialLeadInbox(form); }
export async function closeCommercialLeadInbox(form: FormData) { await closeCommercialLeadInboxAction(form); await revalidateCommercialLeadInbox(form); }
export async function reopenCommercialLeadInbox(form: FormData) { await reopenCommercialLeadInboxAction(form); await revalidateCommercialLeadInbox(form); }
...
export async function runMockAiAndRedirect(form: FormData) { const agentCode=String(form.get('agentCode')||''); const prompt=String(form.get('prompt')||''); const requestKey=String(form.get('requestKey')||''); const supersedesRequestId=String(form.get('supersedesRequestId')||'') || undefined; const request=await runMockAgent(agentCode,{ prompt, source:'CRM interno FAI', humanReviewRequired:true },requestKey,supersedesRequestId); revalidatePath('/settings/ai-authorizations'); revalidatePath('/notifications'); revalidatePath('/dashboard'); redirect(`/settings/ai-authorizations/${request.id}`); }
export async function runClientAiAgentAndRedirect(form: FormData) { const request = await runClientAiAgent(form); if (request.clientId) revalidatePath(`/clients/${request.clientId}`); revalidatePath('/settings/ai-authorizations'); revalidatePath('/notifications'); revalidatePath('/dashboard'); redirect(`/settings/ai-authorizations/${request.id}`); }
