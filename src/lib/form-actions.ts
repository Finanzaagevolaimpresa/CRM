...
export async function runMockAiAndRedirect(form: FormData) { const agentCode=String(form.get('agentCode')||''); const prompt=String(form.get('prompt')||''); const requestKey=String(form.get('requestKey')||''); const request=await runMockAgent(agentCode,{ prompt, source:'CRM interno FAI', humanReviewRequired:true },requestKey); revalidatePath('/settings/ai-authorizations'); revalidatePath('/notifications'); revalidatePath('/dashboard'); redirect(`/settings/ai-authorizations/${request.id}`); }
