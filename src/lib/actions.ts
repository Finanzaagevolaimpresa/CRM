...
export async function updateCommercialOffer(form: FormData) { const s = await requirePermission('lead.write'); const data = commercialOfferUpdateSchema.parse(clean(form)); const before = await prisma.commercialOffer.findUniqueOrThrow({ where: { id: data.id } }); const offer = await prisma.commercialOffer.update({ where: { id: data.id }, data: { leadId: data.leadId ?? null, clientId: data.clientId ?? null, title: data.title, description: data.description, services: data.services, includedActivities: data.includedActivities, taxableAmount: data.taxableAmount, vatAmount: data.vatAmount, totalAmount: data.totalAmount, validUntil: data.validUntil ?? null, operationalConditions: data.operationalConditions, commercialProposal: data.commercialProposal, status: data.status, notes: data.notes, sentAt: data.sentAt ?? null, followUpAt: data.followUpAt ?? null, followUpNote: data.followUpNote, outcomeNote: data.outcomeNote, acceptedAt: data.acceptedAt ?? null, rejectedAt: data.rejectedAt ?? null, rejectionReason: data.rejectionReason } }); const events = ['commercial_offer_update']; if (before.status !== offer.status) events.push('commercial_offer_status_change'); if (!before.sentAt && offer.sentAt) events.push('commercial_offer_sent'); if (before.followUpAt?.getTime() !== offer.followUpAt?.getTime() || before.followUpNote !== offer.followUpNote) events.push('commercial_offer_followup_update'); if (offer.status === 'accettata' && before.status !== offer.status) events.push('commercial_offer_accepted'); if (offer.status === 'rifiutata' && before.status !== offer.status) events.push('commercial_offer_rejected'); if (before.outcomeNote !== offer.outcomeNote || before.acceptedAt?.getTime() !== offer.acceptedAt?.getTime() || before.rejectedAt?.getTime() !== offer.rejectedAt?.getTime() || before.rejectionReason !== offer.rejectionReason) events.push('commercial_offer_outcome_update'); await Promise.all([...new Set(events)].map((event) => audit(s.userId, event, 'CommercialOffer', offer.id, { before, after: offer }))); return offer; }
