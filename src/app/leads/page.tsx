...
export default async function Page() {
  const [items, clientRows, projectRows, userRows] = await Promise.all([prisma.lead.findMany({ where: { deletedAt: null }, orderBy: { updatedAt: 'desc' } }), prisma.client.findMany({ where: { deletedAt: null } }), prisma.project.findMany({ where: { deletedAt: null } }), prisma.user.findMany({ where: { active: true } })]);
