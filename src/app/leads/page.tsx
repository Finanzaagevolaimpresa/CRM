...
export default async function Page() {
  ...
  const visibleItems =
    ...
    ["revisore", "backoffice"].includes(session.role)
      ? items
      : items.filter((x) => x.assignedToId === session.userId);
  return (
    <div className="space-y-6">
      <PageHeader
        title="Lead"
        description="Gestione commerciale dei contatti: stato, interesse, assegnazione e prossima azione."
      />
      <Card title="Crea lead">
        <form
          action={createLeadAndRedirect}
        ...
        >
          ...
          />
          <PrimaryButton type="submit">Crea lead</PrimaryButton>
        </form>
      </Card>
        ...
        {visibleItems.length === 0 ? (
          ...
          <Table
            ...
            rows={visibleItems.map((x) => [
              ...
              x.nextAction
                ? x.nextAction.toISOString().slice(0, 10)
                : "Da pianificare",
              <MetaCell
                key="m"
              ...
              />,
              <Link
                className="font-bold text-fai-blue underline"
                href={`/leads/${x.id}`}
                key="a"
              >
                Apri
