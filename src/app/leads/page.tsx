...
export default async function Page({ searchParams }: { searchParams?: Promise<Record<string, string | undefined>> }) {
  ...
  const where: Prisma.LeadWhereInput = { deletedAt: null, AND: [leadVisibilityWhere(session)] };
  ...
  const [rows, userRows] = await Promise.all([
    prisma.lead.findMany({
      where,
      orderBy: [{ nextActionDate: "asc" }, { updatedAt: "desc" }, { id: "desc" }],
    ...
    prisma.user.findMany({
      ...
      take: CORE_QUERY_REFERENCE_LIMIT,
      select: { id: true, name: true, role: true },
    }),
  ]);
  const page = toCoreQueryPage(rows, pageNumber);
  const visibleItems = page.items;
  const clientIds = [...new Set(visibleItems.map((item) => item.clientId).filter((id): id is string => Boolean(id)))];
  const clientRows = clientIds.length ? await prisma.client.findMany({
  ...
  return <div className="space-y-6"><PageHeader title="Pipeline commerciale lead" description="Gestione interna di lead, fonti, priorità, consulenti assegnati, prossime azioni e conversione verso il fascicolo cliente." />
    {commercialLeadInboxMode() === "enforced" && <SecondaryLink href="/leads/inbox">Apri Commercial Lead Inbox</SecondaryLink>}
    {params.created === "1" && <p role="status" className="rounded-xl bg-green-50 p-3">Lead registrato nella coda dell’amministratore.</p>}
