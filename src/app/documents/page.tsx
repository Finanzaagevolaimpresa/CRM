...
export default async function Page({ searchParams }: { searchParams?: Promise<{ uploadError?: string }> }) {
  ...
  const writableProjects = canUpload ? projectRows.filter((project) => {
    const hydrated = projectById.get(project.id);
    return !!hydrated && canEditProject(session, hydrated);
  }) : [];
  const writableServices = canUpload ? serviceRows.filter((service) => {
    const hydrated = serviceById.get(service.id);
    return !!hydrated && canEditService(session, hydrated);
  }) : [];
  const childClientIds = new Set([...writableProjects, ...writableServices].map(row => row.clientId));
  // A child's assignment exposes only its parent's selector label, not the client dossier.
  const writableClients = canUpload ? clientRows.filter(client => canEditClient(session, client) || childClientIds.has(client.id)) : [];
  const generalClientIds = new Set(clientRows.filter(client => canEditClient(session, client)).map(client => client.id));
  const writableCompanies = canUpload ? companyRows.filter(company => generalClientIds.has(company.clientId)) : [];
