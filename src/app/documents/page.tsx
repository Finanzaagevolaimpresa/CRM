...
export default async function Page({ searchParams }: { searchParams?: Promise<{ uploadError?: string }> }) {
  ...
  const requestText = missingChecklist.length ? `Gentile cliente, per proseguire con la lavorazione della pratica abbiamo necessità di ricevere i seguenti documenti: ${missingChecklist.slice(0, 12).map((item) => `${clients.get(item.clientId) ?? 'Cliente'} - ${item.title}`).join('; ')}. Restiamo a disposizione per eventuali chiarimenti.` : 'Nessun documento mancante risulta richiesto nelle checklist visibili.';
