...
import { UserRemovalForm } from '@/components/user-removal-form';
...
export default async function UserPermissionsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  {session.role === 'admin' ? <Card title="Gestione account"><UserAccountForms user={{ id: user.id, name: user.name, email: user.email }} own={session.userId === user.id} admin available={internalSessionMode() === 'registry'} /></Card> : null}
  {session.role === 'admin' ? <Card title="Perimetro clienti"><Link href={`/settings/users/${user.id}/perimeter`}>Gestisci consultazioni aggiuntive e revoche</Link><p>Consenti la lettura di clienti specifici, mantenendo separate le responsabilità operative.</p></Card> : null}
  {session.role === 'admin' ? <Card title="Rimozione con conservazione dello storico"><UserRemovalForm userId={user.id} disabled={session.userId === user.id || internalSessionMode() !== 'registry'} /></Card> : null}
