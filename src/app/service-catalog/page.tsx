...
import { storedRevisionMatches } from '@/lib/service-catalog-v2-persistence';
...
export default async function Page({ searchParams }: { searchParams: Promise<{ serviceCode?: string; digitalProjectType?: string }> }) {
  ...
  const available = FAI_SERVICE_CATALOG_V2.filter((service) => (historyByCode.get(service.code) ?? []).some((revision) => revision.status === 'PUBLISHED' && storedRevisionMatches(service, revision)));
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-5 lg:grid-cols-2">{FAI_SERVICE_CATALOG_V2.map((service) => {
      ...
      return <Card key={service.code} title={service.name}><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Catalogo {service.sourceCatalogVersion} · revisione {service.revisionVersion} · {service.termsVersion}</p><p className={`mt-2 rounded-lg p-2 text-sm font-bold ${prepared ? 'bg-green-50 text-green-800' : 'bg-amber-50 text-amber-900'}`}>{prepared ? 'Revisione disponibile nel CRM' : 'Revisione non ancora preparata nel CRM'}</p><p className="mt-1 font-black text-fai-blue">{priceLabel(service.priceMode, service.netPriceCents)}</p><p className="mt-2 text-sm text-slate-600">{service.description}</p><h3 className="mt-4 font-bold">Inclusioni</h3><ul className="list-disc pl-5 text-sm">{service.detail.inclusions.map((item) => <li key={item}>{item}</li>)}</ul><h3 className="mt-4 font-bold">Esclusioni</h3><ul className="list-disc pl-5 text-sm">{service.detail.exclusions.map((item) => <li key={item}>{item}</li>)}</ul><h3 className="mt-4 font-bold">Documenti e fasi</h3><ul className="list-disc pl-5 text-sm">{service.detail.phaseDocuments.map((item) => <li key={item}>{item}</li>)}</ul><p className="mt-4 text-sm"><strong>Tempi:</strong> {service.detail.timing}</p><h3 className="mt-4 font-bold">Deliverable</h3><ul className="list-disc pl-5 text-sm">{service.detail.deliverables.map((item) => <li key={item}>{item}</li>)}</ul><h3 className="mt-4 font-bold">Incarichi successivi</h3><ul className="list-disc pl-5 text-sm">{service.detail.subsequentEngagements.map((item) => <li key={item}>{item}</li>)}</ul>{service.detail.professionalAttribution ? <p className="mt-3 rounded-xl bg-fai-blue/5 p-3 text-sm">{service.detail.professionalAttribution}</p> : null}{service.detail.digitalProjectTypes ? <><h3 className="mt-4 font-bold">Tipologie disponibili</h3>{service.detail.digitalProjectTypes.map((item) => <section key={item.code} className="mt-3 rounded-xl border p-3"><h4 className="font-bold">{item.label}</h4><p className="text-sm"><strong>Materiali:</strong> {item.materials.join('; ')}</p><p className="text-sm"><strong>Inclusioni:</strong> {item.inclusions.join('; ')}</p><p className="text-sm"><strong>Esclusioni:</strong> {item.exclusions.join('; ')}</p><p className="text-sm"><strong>Deliverable:</strong> {item.deliverables.join('; ')}</p><p className="text-sm"><strong>Incarichi successivi:</strong> {item.subsequentEngagements.join('; ')}</p></section>)}</> : null}<h3 className="mt-4 font-bold">Storico revisioni</h3>{(historyByCode.get(service.code) ?? []).length ? <ul className="text-sm">{historyByCode.get(service.code)!.map((revision) => <li key={revision.id}>Versione {revision.version} · {revision.termsVersion} · {revision.priceMode === 'QUOTE_ONLY' ? 'Su preventivo' : `€ ${Number(revision.netPrice).toLocaleString('it-IT')} + IVA`} · {revision.status === 'PUBLISHED' ? 'corrente' : 'storica'}</li>)}</ul> : <EmptyState title="Nessuna revisione persistita" />}</Card>;
    })}</div>
  </div>;
}
