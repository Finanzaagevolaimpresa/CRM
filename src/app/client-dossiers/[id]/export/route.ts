...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (dossier.practiceReadinessId) {
    ...
    return new NextResponse(version.content, { headers: { 'Content-Type': 'text/markdown; charset=utf-8', 'Content-Disposition': `attachment; filename="${safeFileName(version.title)}-v${version.version}.md"`, 'X-Dossier-Version': String(version.version), 'X-Dossier-Content-Hash': version.contentHash } });
  }
