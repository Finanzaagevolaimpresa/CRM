...
import { assertClassifiedFields, UnclassifiedDataFieldError } from '@/lib/data-classification';
import {
  createWebsiteLeadPrivacyEvidence,
  PrivacyContractUnavailableError,
  WEBSITE_LEAD_MARKETING_LEGAL_BASIS,
  WEBSITE_LEAD_MARKETING_PURPOSE,
  WEBSITE_LEAD_PRIVACY_LEGAL_BASIS,
  WEBSITE_LEAD_PRIVACY_PURPOSE,
} from '@/lib/privacy-evidence';
...
const contractCode = z.string().trim().min(1).max(120).regex(/^[A-Z0-9][A-Z0-9_.:-]*$/);
const contractVersion = z.string().trim().min(1).max(80).regex(/^[A-Za-z0-9][A-Za-z0-9_.:-]*$/);
const websiteLeadSchema = z.object({
  ...
  sourceSystem: contractCode,
  formCode: contractCode,
  formVersion: contractVersion,
  privacyAccepted: z.literal(true),
  privacyNoticeCode: contractCode,
  privacyNoticeVersion: contractVersion,
  privacyPurposeCode: z.literal(WEBSITE_LEAD_PRIVACY_PURPOSE),
  privacyLegalBasisCode: z.literal(WEBSITE_LEAD_PRIVACY_LEGAL_BASIS),
  marketingAccepted: z.boolean(),
  marketingNoticeCode: contractCode,
  marketingNoticeVersion: contractVersion,
  marketingPurposeCode: z.literal(WEBSITE_LEAD_MARKETING_PURPOSE),
  marketingLegalBasisCode: z.literal(WEBSITE_LEAD_MARKETING_LEGAL_BASIS),
...
function canonicalPayload(data: Input) {
  return `website-lead-payload:v2\n${JSON.stringify({ firstName:data.firstName,lastName:data.lastName,companyName:data.companyName,email:data.email,phone:data.phone,city:data.city,region:data.region,interest:data.interest,requestedAmount:data.requestedAmount,message:data.message,sourcePage:data.sourcePage,serviceInterest:data.serviceInterest,sourceSystem:data.sourceSystem,formCode:data.formCode,formVersion:data.formVersion,privacyAccepted:data.privacyAccepted,privacyNoticeCode:data.privacyNoticeCode,privacyNoticeVersion:data.privacyNoticeVersion,privacyPurposeCode:data.privacyPurposeCode,privacyLegalBasisCode:data.privacyLegalBasisCode,marketingAccepted:data.marketingAccepted,marketingNoticeCode:data.marketingNoticeCode,marketingNoticeVersion:data.marketingNoticeVersion,marketingPurposeCode:data.marketingPurposeCode,marketingLegalBasisCode:data.marketingLegalBasisCode,submittedAt:data.submittedAt })}`;
}
...
async function processLegacy(data: Input, keyDigest: string, payloadHash: string, deadline: WebsiteLeadDeadline) {
  return runWebsiteLeadTransactionWithRetry(deadline, async (timeout) =>
    prisma.$transaction(async (tx) => {
      ...
      const privacyEvidence = await transactionOperation(tx, deadline, () => createWebsiteLeadPrivacyEvidence(tx, {
        leadId: lead.id,
        websiteLeadReceiptId: receipt.id,
        sourceEvidenceDigest: payloadHash,
        sourceSystem: data.sourceSystem,
        formCode: data.formCode,
        formVersion: data.formVersion,
        sourceSubmittedAt: new Date(data.submittedAt),
        privacyAccepted: data.privacyAccepted,
        privacyNoticeCode: data.privacyNoticeCode,
        privacyNoticeVersion: data.privacyNoticeVersion,
        privacyPurposeCode: data.privacyPurposeCode,
        privacyLegalBasisCode: data.privacyLegalBasisCode,
        marketingAccepted: data.marketingAccepted,
        marketingNoticeCode: data.marketingNoticeCode,
        marketingNoticeVersion: data.marketingNoticeVersion,
        marketingPurposeCode: data.marketingPurposeCode,
        marketingLegalBasisCode: data.marketingLegalBasisCode,
      }));
      await transactionOperation(tx, deadline, () => tx.auditLog.create({ data: { event: duplicate ? 'website_lead_duplicate_detected' : 'website_lead_received', entityType: 'Lead', entityId: lead.id, after: { mode:'legacy', outcome:duplicate ? 'updated' : 'created', contractVersion:'v2', catalogVersion:'n04-v1', privacyEvidenceCount:privacyEvidence.count, receipt:receipt.id } } }));
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try { assertClassifiedFields('website_lead_intake_v2', parsed.data); }
    catch (error) { if (error instanceof UnclassifiedDataFieldError) return response(400); throw error; }
    if (mode === 'shadow') {
      try { integrationEnabled = await isApplicationFeatureEnabled(prisma, 'INTEGRATIONS'); }
      catch { return unavailable(); }
    }
    ...
    if (error instanceof PrivacyContractUnavailableError || error instanceof UnclassifiedDataFieldError) return response(400);
