...
export default async function Page() {
  ...
  const visibleProjectRows = projectRows.filter((project) => canViewProject(session, { ...project, client: clientById.get(project.clientId) ?? null }));
  const visibleProjectIds = new Set(visibleProjectRows.map((project) => project.id));
  const visibleItems = items.filter((item) => visibleClientIds.has(item.clientId) && visibleProjectIds.has(item.projectId));
  const clients = new Map(visibleClientRows.map((c) => [c.id, c.displayName]));
  const projects = new Map(visibleProjectRows.map((p) => [p.id, p.title]));
  return <div className="space-y-6"><PageHeader title="Pre-analisi" description="Bozze interne da revisionare: nessun output viene considerato approvato senza controllo umano."/>{canWrite ? <Card title="Crea pre-analisi"><form action={createPreAnalysisAndRedirect} className="grid gap-3 md:grid-cols-4"><select className="rounded-xl border p-3" name="clientId" required>{visibleClientRows.map(c=><option key={c.id} value={c.id}>{c.displayName}</option>)}</select><select className="rounded-xl border p-3" name="projectId" required>{visibleProjectRows.map(p=><option key={p.id} value={p.id}>{p.title}</option>)}</select><input className="rounded-xl border p-3 md:col-span-2" name="internalSummary" placeholder="Sintesi interna"/><PrimaryButton type="submit" className="md:col-span-4">Crea pre-analisi</PrimaryButton></form></Card> : null}<Card title="Elenco operativo">{visibleItems.length === 0 ? <EmptyState title="Nessun elemento presente">Non ci sono record da lavorare per questa sezione.</EmptyState> : <Table headers={['Cliente', 'Progetto', 'Stato', 'Sintesi', 'Tracciabilità', 'Azione']} rows={visibleItems.map((x) => [clients.get(x.clientId) ?? '—', projects.get(x.projectId) ?? '—', <StatusBadge status={x.status} key='s' />, x.internalSummary ?? 'Bozza interna', <MetaCell key='m' createdAt={x.createdAt} updatedAt={x.updatedAt} />, <Link className='font-bold text-fai-blue underline' href={`/preanalyses/${x.id}`} key='a'>Apri</Link>])} />}</Card></div>;
}
