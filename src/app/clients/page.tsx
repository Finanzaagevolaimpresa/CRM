...
export default async function Page() {
  ...
  const [clientRows, users, serviceCounts, documentCounts, latestServices] = await Promise.all([
    ...
    prisma.clientService.findMany({ where: { deletedAt: null, operationalStatus: { notIn: ['chiusa','archiviata'] } }, orderBy: { statusUpdatedAt: 'desc' }, select: { clientId: true, operationalStatus: true, statusUpdatedAt: true } }),
  ]);
  ...
  const mainStatus = (clientId: string) => latestServices.find((service) => service.clientId === clientId)?.operationalStatus;
