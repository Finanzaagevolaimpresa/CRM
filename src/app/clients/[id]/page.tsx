...
import { assignServiceAndRefresh, updateServiceStatusAndRefresh } from '@/lib/form-actions';
...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="space-y-6">
    <PageHeader title={`Fascicolo Cliente Interno — ${client.displayName}`} description="Scheda operativa interna FAI: servizi acquistati, documenti per sezione, output AI in bozza con revisione umana obbligatoria e audit."/><SecondaryLink href="/clients">← Torna alla lista</SecondaryLink>
    <nav className="sticky top-20 z-10 flex flex-wrap gap-2 rounded-2xl border border-slate-200 bg-white/95 p-3 shadow-sm backdrop-blur">{serviceSections.map(([id, label]) => <a className="rounded-full bg-fai-blue/10 px-3 py-1.5 text-xs font-bold text-fai-blue hover:bg-fai-blue hover:text-white" href={`#${id}`} key={id}>{label}</a>)}</nav>

    <Card id="overview" title="Overview"><div className="grid gap-4 md:grid-cols-4"><div><p className="font-semibold">{client.displayName}</p><p className="text-sm text-fai-gray">Tipo: {client.type}</p><p className="text-sm text-fai-gray">Stato: {client.status}</p></div><p className="text-3xl font-bold text-fai-blue">{companies.length}<span className="block text-xs text-fai-gray">Aziende</span></p><p className="text-3xl font-bold text-fai-blue">{projects.length}<span className="block text-xs text-fai-gray">Progetti</span></p><p className="text-3xl font-bold text-fai-blue">{clientServices.length}<span className="block text-xs text-fai-gray">Servizi</span></p></div><TimestampMeta createdAt={client.createdAt} updatedAt={client.updatedAt} createdBy={userOf(client.salesOwnerId)} updatedBy={userOf(client.consultantId)} /></Card>
    ...
    <Card id="servizi-acquistati" title="Servizi acquistati"><div className="grid gap-4 md:grid-cols-2">{clientServices.map((s) => <article id={`service-${s.id}`} key={s.id} className="scroll-mt-36 rounded-xl border p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="font-semibold text-fai-navy">{nameOf(s.serviceCatalogId)}</h3><p className="text-sm text-fai-gray">Responsabile: {userOf(s.assignedToId)}</p></div><div className="flex gap-2"><StatusBadge status={s.paymentStatus} /><StatusBadge status={s.status} /></div></div><TimestampMeta createdAt={s.createdAt} updatedAt={s.updatedAt} updatedBy={userOf(s.assignedToId)} /><p className="mt-3 text-sm">Note interne: {s.internalNotes ?? '—'}</p><div className="mt-3 grid gap-2 md:grid-cols-2"><form action={updateServiceStatusAndRefresh} className="flex gap-2"><input type="hidden" name="id" value={s.id}/><select name="status" defaultValue={s.status} className="min-w-0 flex-1 rounded-lg border px-2 py-1">{['richiesto','pagato','raccolta_documenti','in_lavorazione','bozza_ai','revisione_umana','consegnabile','consegnato','sospeso','chiuso','archiviato'].map(st=><option key={st} value={st}>{st}</option>)}</select><PrimaryButton type="submit">Salva</PrimaryButton></form><form action={assignServiceAndRefresh} className="flex gap-2"><input type="hidden" name="id" value={s.id}/><select name="assignedToId" defaultValue={s.assignedToId??''} className="min-w-0 flex-1 rounded-lg border px-2 py-1"><option value="">Da assegnare</option>{users.map(u=><option key={u.id} value={u.id}>{u.name}</option>)}</select><PrimaryButton type="submit">Assegna</PrimaryButton></form></div><p className="mt-2 text-sm text-fai-gray">Checklist documentale: {checklist.join(' · ')}</p></article>)}</div>{clientServices.length === 0 && <EmptyState title="Nessun servizio acquistato" />}</Card>
    ...
    <Card id="documenti" title="Documenti">{documents.length === 0 ? <EmptyState title="Nessun documento" /> : <Table headers={['Documento','Sezione','Categoria','Stato','Creato il','Aggiornato il']} rows={documents.map((d) => [d.title, d.serviceArea, d.documentCategory, <StatusBadge status={d.status} key='s' />, formatDateTime(d.createdAt), formatDateTime(d.updatedAt)])} />}</Card>
