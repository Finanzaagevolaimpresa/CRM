...
import { getSession, hasPermission, permissionCodes } from "@/lib/auth";
...
export default async function RootLayout({
  ...
  const notificationCount = session
    ...
    : 0;
  const effectivePermissions = session ? permissionCodes.filter((permission) => hasPermission(session, permission)) : [];
  return (
    <html lang="it">
      <body>
        <div className="flex min-h-screen min-w-0 flex-col md:h-screen md:overflow-hidden md:flex-row">
          <Nav notificationCount={notificationCount} permissions={effectivePermissions} role={session?.role} />
