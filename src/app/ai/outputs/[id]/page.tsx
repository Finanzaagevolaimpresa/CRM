...
import { getAiOutputReadAccess } from '@/lib/read-access';

export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  const session = await requirePermission('ai.review');
  const { id } = await params;
  const context = await getAiOutputReadAccess(session, id);
  if (!context) return <h1 className="text-3xl font-bold text-fai-navy">Output AI non trovato o non accessibile</h1>;
  const { output, run, clientService: service, project } = context;
  const [client, agent] = await Promise.all([
    output.clientId ? prisma.client.findUnique({ where: { id: output.clientId } }) : null,
    prisma.aiAgent.findUnique({ where: { id: run.agentId } }),
  ]);
  ...
  return <div className="space-y-6">
    ...
    <Card title="Contenuto interno"><pre className="whitespace-pre-wrap text-sm leading-6 text-slate-700">{output.content}</pre></Card>
  </div>;
}
