...
type DocumentUploadFormProps = {
  clients: Array<Option & { generalUploadAllowed?: boolean }>;
  companies: Option[];
  projects: Option[];
  services: Option[];
  serviceAreas: string[];
...
export function DocumentUploadForm({ clients, companies, projects, services, serviceAreas, fixedClientId, submitLabel = 'Carica in storage privato', className = 'grid gap-3 md:grid-cols-4', buttonClassName = 'md:col-span-4', includeProject = true }: DocumentUploadFormProps) {
  ...
  const needsChild = clients.find(client => client.id === selectedClientId)?.generalUploadAllowed === false;
  ...
  return <form action={uploadDocumentAndRefresh} className={className}>
    {fixedClientId ? <input type="hidden" name="clientId" value={fixedClientId} /> : <select className="rounded-xl border p-3" name="clientId" required value={selectedClientId} onChange={(event) => { setSelectedClientId(event.target.value); setSelectedProjectId(''); setSelectedServiceId(''); }}><option value="">Cliente</option>{clients.map((client) => <option key={client.id} value={client.id}>{client.label}</option>)}</select>}
    ...
    {includeProject ? <select className="rounded-xl border p-3" name="projectId" value={selectedProjectId} onChange={event => setSelectedProjectId(event.target.value)} disabled={relationDisabled}><option value="">Progetto opzionale</option>{filteredProjects.map((project) => <option key={project.id} value={project.id}>{project.label}</option>)}</select> : null}
    ...
    <select className="rounded-xl border p-3" name="clientServiceId" value={selectedServiceId} onChange={event => setSelectedServiceId(event.target.value)} disabled={relationDisabled}><option value="">Fascicolo generale</option>{filteredServices.map((service) => <option key={service.id} value={service.id}>{service.label}</option>)}</select>
    ...
    {needsChild ? <p role="status" className="md:col-span-4">Per questo cliente puoi caricare solo nel progetto o servizio assegnato. Seleziona il contesto.</p> : null}
    <PrimaryButton type="submit" disabled={needsChild && !selectedProjectId && !selectedServiceId} className={buttonClassName}>{submitLabel}</PrimaryButton>
  </form>;
}
