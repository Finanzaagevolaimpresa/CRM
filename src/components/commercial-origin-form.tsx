...
export function CommercialOriginForm({ clientId, entryId, current, users }: {
  ...
  return <form action={action} aria-label="Provenienza commerciale" className="grid gap-3 md:grid-cols-2">
    ...
    <label>Acquisizione cliente<select aria-label="Acquisizione cliente" name="acquiredById" defaultValue={current?.acquiredById ?? ''} className="block w-full rounded-xl border p-2"><option value="">Non documentata</option>{options}</select></label>
    <label>Contrattualizzazione<select aria-label="Contrattualizzazione" name="contractedById" defaultValue={current?.contractedById ?? ''} className="block w-full rounded-xl border p-2"><option value="">Non documentata</option>{options}</select></label>
