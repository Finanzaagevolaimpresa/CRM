...
export function Card({ title, children, action, id }: { title: string; children: React.ReactNode; action?: React.ReactNode; id?: string }) { return <section id={id} className="scroll-mt-28 rounded-3xl border border-slate-200/75 bg-white/92 p-5 shadow-sm shadow-slate-200/70 ring-1 ring-slate-900/5 backdrop-blur"><div className="mb-4 flex items-start justify-between gap-3"><div><p className="mb-1 h-1 w-10 rounded-full bg-gradient-to-r from-fai-lime to-fai-orange" /><h2 className="text-lg font-extrabold tracking-tight text-fai-navy">{title}</h2></div>{action}</div>{children}</section>; }
...
export function Stat({ label, value, description, href, tone = 'blue' }: { label: string; value: number | string; description?: string; href?: string; tone?: 'blue' | 'green' | 'orange' | 'purple' | 'lime' }) { const toneClass = { blue: 'from-fai-blue to-fai-navy', green: 'from-fai-green to-fai-teal', orange: 'from-fai-orange to-amber-500', purple: 'from-fai-purple to-fai-blue', lime: 'from-fai-lime to-fai-green' }[tone]; const box = <div className="group relative flex h-full min-h-32 flex-col overflow-hidden rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm shadow-slate-200/70 ring-1 ring-slate-900/5 transition duration-200 hover:-translate-y-0.5 hover:border-fai-blue/25 hover:shadow-md"><div className={`absolute inset-x-0 top-0 h-1 bg-gradient-to-r ${toneClass}`} /><div className={`bg-gradient-to-br ${toneClass} bg-clip-text text-3xl font-black tracking-tight text-transparent`}>{value}</div><div className="mt-2 text-sm font-extrabold text-fai-navy">{label}</div>{description && <div className="mt-1 text-xs leading-5 text-slate-500">{description}</div>}{href && <div className="mt-auto pt-3 text-[0.68rem] font-black uppercase tracking-wide text-fai-green">Apri →</div>}</div>; return href ? <Link href={href} className="block h-full">{box}</Link> : box; }
...
export function PageHeader({ title, description }: { title: string; description: string }) { return <header className="relative overflow-hidden rounded-3xl bg-fai-navy p-5 text-white shadow-lg shadow-fai-blue/10"><div className="absolute inset-0 bg-[linear-gradient(135deg,#052E70,#043E8A_48%,#00683E)]" /><div className="relative flex flex-col gap-2 md:flex-row md:items-end md:justify-between"><div><p className="mb-2 inline-flex rounded-full bg-white/10 px-3 py-1 text-[0.65rem] font-black uppercase tracking-[0.2em] text-fai-lime ring-1 ring-white/15">Control center FAI</p><h1 className="max-w-4xl text-2xl font-black tracking-tight md:text-3xl">{title}</h1></div><p className="max-w-3xl text-xs leading-6 text-white/82 md:text-right">{description}</p></div></header>; }

export function Nav({ role }: { role?: import('@prisma/client').RoleCode | null }) { return <aside className="sticky top-0 z-30 flex max-h-screen w-full shrink-0 flex-col overflow-y-auto bg-fai-navy p-4 text-white shadow-xl shadow-fai-navy/20 md:min-h-screen md:w-64"><div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_20%_0%,rgba(128,204,42,.16),transparent_28%),radial-gradient(circle_at_100%_35%,rgba(61,41,116,.28),transparent_28%)]" /><div className="relative flex min-h-0 flex-1 flex-col"><Link href="/dashboard" className="mb-5 flex items-center gap-3 rounded-2xl border border-white/10 bg-white p-2.5 shadow-md"><span className="relative grid h-11 w-[72px] shrink-0 place-items-center rounded-xl bg-slate-50 text-xs font-black text-fai-navy ring-1 ring-slate-200"><span>FAI</span><img src="/logo-fai.png" alt="Finanza Agevola Impresa" className="absolute inset-0 h-full w-full rounded-xl bg-white object-contain p-1" /></span><span className="min-w-0"><span className="block text-sm font-black text-fai-navy">Gestionale CRM</span><span className="block truncate text-xs font-bold text-slate-500">Finanza Agevola Impresa</span></span></Link><div className="min-h-0 flex-1 pr-1"><NavLinks role={role} /></div><form action={logoutAction} className="mt-5 border-t border-white/15 pt-4"><button className="w-full rounded-xl bg-white/10 px-4 py-2.5 text-left text-sm font-bold text-white transition hover:bg-fai-orange focus:outline-none focus:ring-2 focus:ring-fai-lime" type="submit">Logout</button></form></div></aside>; }
